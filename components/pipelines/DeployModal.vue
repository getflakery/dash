...
import type { Pipeline, Network } from '~/types';
...
const deployInstance = async (id: string, refresh: () => void) => {
  ...
  useToast().add({
    ...
    description: `Your pipeline ${props.pipeline.name}  has been deployed as ${inst.name}.`,
    timeout: 5000,
    click: () => {
...
<template>
  ...
  <div class="flex justify-end gap-4">
    ...
    <UButton icon="i-heroicons-paper-airplane" :loading="deploying" type="submit" label="Run Pipeline"
