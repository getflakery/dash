      ...
      >
        <slot :item="item" :index="index" :open="open" :close="close">
          ...
          :loading="item.loading"
           v-bind="{ ...omit(ui.default, ['openIcon', 'closeIcon',]), ...attrs, ...omit(item, ['slot', 'disabled', 'content', 'defaultOpen']) }">
            <template #trailing>
              <UIcon
                v-if="item.content"
...
// import UButton from '../ui/src/runtime/components/elements/Button.vue'
import { useUI } from '../ui/src/runtime/composables/useUI'
import { mergeConfig, omit } from '../ui/src/runtime/utils'
import type { AccordionItem, Strategy } from '../ui/src/runtime/types'
// @ts-expect-error
